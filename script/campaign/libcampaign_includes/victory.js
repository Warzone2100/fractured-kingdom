...
// Almost everything was ripped from this function.
// Now all it does is read the time and call a function in the main level script.
function __camShowVictoryConditions()
{
	if (__camDayCycleActive)
	{
		const __SECONDS_IN_A_DAY_CYCLE_HOUR = (__CAM_SECONDS_IN_A_DAY_CYCLE / 24);
		// Get the in-game time
		const __DAY_CYCLE_MINUTES = Math.floor(60 * (camGetDayTime() % __SECONDS_IN_A_DAY_CYCLE_HOUR) / __SECONDS_IN_A_DAY_CYCLE_HOUR);
		let dayCycleHours = Math.floor(camGetDayTime() / __SECONDS_IN_A_DAY_CYCLE_HOUR) + 6;
		if (dayCycleHours >= 24)
		{
			dayCycleHours -= 24;
		}

		let displayHours = dayCycleHours;
		let displayMinutes = __DAY_CYCLE_MINUTES;
		// Put a 0 in front of the time if it has only 1 digit.
		if (dayCycleHours < 10)
		{
			displayHours = "0" + dayCycleHours;
		}
		if (__DAY_CYCLE_MINUTES < 10)
		{
			displayMinutes = "0" + __DAY_CYCLE_MINUTES;
		}

		console(_("Current Time: ") + displayHours + ":" + displayMinutes);
	}

	if (camDef(__camVictoryData) && camDef(__camVictoryData.mesCallback))
	{
		__camGlobalContext()[__camVictoryData.mesCallback]();
	}
}

...
//;; * ```mesCallback``` A function that is called whenever victory conditions are shown.
...
function __camShowVictoryConditions()
{
	...
	{
		var secondsInADayHour = (CAM_SECONDS_IN_A_DAY_CYCLE / 24);
		// Get the in-game time
		var dayMinutes = Math.floor(60 * (camGetDayTime() % secondsInADayHour) / secondsInADayHour);
		var dayHours = Math.floor(camGetDayTime() / secondsInADayHour) + 6;
		if (dayHours >= 24)
		{
			dayHours -= 24;
		}

		var displayHours = dayHours;
		var displayMinutes = dayMinutes;
		// Put a 0 in front of the time if it has only 1 digit.
		if (dayHours < 10)
		{
			displayHours = "0" + dayHours;
		}
		if (dayMinutes < 10)
		{
			displayMinutes = "0" + dayMinutes;
		}

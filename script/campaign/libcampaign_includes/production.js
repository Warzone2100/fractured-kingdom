...
//;; ## camSetFactoryTemplates(factory label, template list[, throttle])
//;;
//;; Change the templates that a factory will produce.
//;; This function does not enable or disable factories.
//;; The factory's throttle can also be changed as well.
//;;
function camSetFactoryTemplates(flabel, tList, throttle)
{
	var fi = __camFactoryInfo[flabel];
	if (!camDef(fi))
	{
		camDebug("Factory", flabel, "has not been defined.");
		return;
	}
	fi.templates = tList;
...
//;; ## camDisableFactory(factory label)
//;;
//;; Disable a managed factory by the given label.
//;;
function camDisableFactory(flabel)
{
	var fi = __camFactoryInfo[flabel];
	if (!camDef(fi) || !fi)
	{
	...
	{
		// safe, no error
		camTrace("Factory", flabel, "disabled again");
		return;
	}
	camTrace("Disabling", flabel);
	fi.enabled = false;
	var obj = getObject(flabel);
	if (!camDef(obj) || !obj)
	{
...
//;; ## camQueueDroidProduction(player, template[, position])
//;;
...
//;; its production loop, first queued first served. If a position is given, then 
//;; then ensure the produced droid can reach that position.
//;;
function camQueueDroidProduction(player, template, position)
{
	if (!camDef(__camFactoryQueue[player]))
	{
		__camFactoryQueue[player] = [];
	}
	__camFactoryQueue[player][__camFactoryQueue[player].length] = {template: template, position: position};
}
...
function __camFactoryUpdateTactics(flabel)
{
	...
	if (!fi.enabled)
	{
		// Since factories are frequently turned on or off, this is not an error.
		camTrace("Factory", flabel, "was not enabled");
		// return;
	}
...
function __camBuildDroid(template, structure)
{
	...
	if (template.prop === __camChangePropulsionOnDiff("V-Tol") && structure.stattype !== VTOL_FACTORY)
	{
		// If not a VTOL factory and the template is a VTOL then keep it in the
		// queue until a factory can deal with it.
		return false;
	}
	if (template.prop === __camChangePropulsionOnDiff("CyborgLegs") && structure.stattype !== CYBORG_FACTORY)
	{
	...
	if ((template.prop === __camChangePropulsionOnDiff("wheeled01") || template.prop === __camChangePropulsionOnDiff("HalfTrack") 
		|| template.prop === __camChangePropulsionOnDiff("tracked01") || template.prop === __camChangePropulsionOnDiff("hover01")) 
	...
	var n = camNameTemplate(template);
...
function __camContinueProduction(structure)
{
	...
	if (camDef(__camFactoryQueue[p]) && __camFactoryQueue[p].length > 0)
	{
		...
		var destPos = __camFactoryQueue[p][0].position;
		if ((!camDef(destPos) || propulsionCanReach(__camFactoryQueue[p][0].template.prop, structPos.x, structPos.y, destPos.x, destPos.y)) 
			&& __camBuildDroid(__camFactoryQueue[p][0].template, struct))
		{
	...
	if (camDef(fi.templates[fi.state]) && fi.templates[fi.state] !== null)
	{

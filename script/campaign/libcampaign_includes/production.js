...
//;; ## camSetFactoryTemplates(factoryLabel, template list[, throttle])
//;;
...
//;; @returns {void}
//;;
function camSetFactoryTemplates(factoryLabel, tList, throttle)
{
	const fi = __camFactoryInfo[factoryLabel];
	if (!camDef(fi))
	{
		camDebug("Factory", factoryLabel, "has not been defined.");
		return;
	}
	fi.templates = tList;
...
function __camFactoryUpdateTactics(factoryLabel)
{
	const fi = __camFactoryInfo[factoryLabel];
	if (!fi.enabled)
	{
		// Since factories are frequently turned on or off, this is not an error.
		camTrace("Factory", factoryLabel, "was not enabled");
		// return;
	}
...
{
	for (const factoryLabel in __camFactoryInfo)
	{
		if (getObject(factoryLabel) !== null)
		{
			__camContinueProduction(factoryLabel);
		}
	}
...
function __camContinueProduction(structure)
{
	...
	{
		factoryLabel = structure;
		struct = getObject(factoryLabel);
	...
	{
		// FIXME: O(n) lookup here
		factoryLabel = getLabel(structure);
		struct = structure;
	}
	if (!camDef(factoryLabel) || !factoryLabel)
	{
		return;
	}
